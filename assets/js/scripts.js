...
checkoutBtn.addEventListener("click", function () {
    ...
    // Remove o código do país (se houver) e espaços extras do número
    phoneNumber = phoneNumber.replace(/\D/g, ""); // Remove caracteres não numéricos
    if (phoneNumber.startsWith("55")) {
        phoneNumber = phoneNumber.slice(2); // Remove o "55" caso esteja no início
    }

    // Validação: verifica se o telefone tem 10 ou 11 dígitos (9 dígitos mais o DDD para Brasil)
    if (phoneNumber.length < 10 || phoneNumber.length > 11) {
        alert("Por favor, insira um número de telefone válido com DDD.");
        return;
    }

    const message = `Pedido:\n\nItens:\n${cartItems}\n\nTotal: R$${cartTotalText}\n\nEndereço de entrega: ${address}\n\nTelefone de contato: ${phoneNumber}`;
